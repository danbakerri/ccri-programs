...
let currentPage = 1;
let filteredCards = [];
let state = { q: '', pathway: '', type: '', az: '', workforceCategory: '', location: '' };
...
function buildFilters() {
  ...
  document.querySelectorAll('.program-type-tag').forEach(el => {
    el.textContent = PROGRAM_TYPE_LABELS[el.textContent.trim()] || el.textContent.trim();
  });
  ...
  const visibleTotal = allCards.filter(c => !HIDDEN_PROGRAMS.includes(c.dataset.url || '')).length;
  document.getElementById('total').textContent = visibleTotal;

  /* Count by each dimension */
  const pathwayCounts = {}, typeCounts = {}, azCounts = {}, workforceCategoryCounts = {}, locationCounts = {};
  allCards.forEach(c => {
    ...
    const locs = c.dataset.locations ? c.dataset.locations.split(',').map(l => l.trim()).filter(Boolean) : [];
    ...
    locs.forEach(loc => { locationCounts[loc] = (locationCounts[loc] || 0) + 1; });
  });
  ...
  const workforceCategories = [
    'Business and Technology', 'Education', 'Healthcare',
    'Manufacturing and Trades', 'Renewable Energy', 'Fit2Serve',
    'GED & Adult Education', 'Transportation Education'
  ];
...
function applyState(s, page, shouldScroll) {
  ...
  state = { q: s.q, pathway: s.pathway, type: s.type, az: s.az, workforceCategory: s.workforceCategory, location: s.location };
...
function _filter() {
  ...
  filteredCards = allCards.filter(c => {
    ...
    if (state.location) {
      const locs = c.dataset.locations ? c.dataset.locations.split(',').map(l => l.trim()) : [];
  ...
  clearBtn.classList.toggle('visible',
    !!(state.q || state.pathway || state.type || state.az || state.workforceCategory || state.location));
}
...
function renderChips() {
  ...
  chipsEl.innerHTML = '';
  if (state.q)        chipsEl.appendChild(makeChip('Search: ' + state.q, () => { state.q = ''; qInput.value = ''; goSearch(); }));
  if (state.pathway)  chipsEl.appendChild(makeChip(state.pathway,         () => { pathwaySel.value = ''; go(); }));
  if (state.type)     chipsEl.appendChild(makeChip(PROGRAM_TYPE_LABELS[state.type] || state.type, () => { typeSel.value = ''; go(); }));
  if (state.workforceCategory) chipsEl.appendChild(makeChip('Category: ' + state.workforceCategory, () => { workforceCategorySel.value = ''; go(); }));
  if (state.location) chipsEl.appendChild(makeChip(locationLabels[state.location] || state.location, () => { locationSel.value = ''; go(); }));
  if (state.az)       chipsEl.appendChild(makeChip('A–Z: ' + state.az,   () => { azSel.value = ''; go(); }));
}
...
function clearAll() {
  state = { q: '', pathway: '', type: '', az: '', workforceCategory: '', location: '' };
  qInput.value                   = '';
  pathwaySel.value               = '';
  typeSel.value                  = '';
  azSel.value                    = '';
...
function renderPager() {
  ...
  if (wS > 1) { pager.appendChild(makePageBtn(1)); if (wS > 2) pager.appendChild(makeDots()); }
  for (let i = wS; i <= wE; i++) pager.appendChild(makePageBtn(i));
  if (wE < pages) { if (wE < pages - 1) pager.appendChild(makeDots()); pager.appendChild(makePageBtn(pages)); }
